...
export async function exportDocument({
  ...
  locale,
  format,
}: ExportDocumentOptions): Promise<{ blob: Blob; fileName: string }> {
  const editorHtml = sanitizeDocumentHtml(html);
  const blocks = extractBlocksFromHtml(editorHtml);

  switch (format) {
    case 'txt':
      return {
        blob: new Blob([blocksToPlainText(blocks)], { type: 'text/plain' }),
        fileName: buildExportFileName(name, 'txt'),
      };
    case 'html':
      return {
        blob: new Blob([buildHtmlDocument(editorHtml, name, locale)], { type: 'text/html' }),
        fileName: buildExportFileName(name, 'html'),
      };
    case 'rtf':
      return {
        blob: new Blob([convertHtmlToRtf(editorHtml)], { type: 'application/rtf' }),
        fileName: buildExportFileName(name, 'rtf'),
      };
    case 'docx':
      return {
        blob: await buildDocxBlob(blocks),
        fileName: buildExportFileName(name, 'docx'),
      };
    case 'odt':
      return {
        blob: await buildOdtBlob(blocks),
        fileName: buildExportFileName(name, 'odt'),
      };
    case 'pdf':
      return {
        blob: await buildPdfBlob(blocks),
        fileName: buildExportFileName(name, 'pdf'),
      };
  }
}

function buildHtmlDocument(editorHtml: string, documentName: string, locale: Locale): string {
  return `<!DOCTYPE html>
<html lang="${locale}">
<head>
  <meta charset="UTF-8">
...
async function buildOdtBlob(blocks: HtmlBlock[]): Promise<Blob> {
...
function buildDocxDocumentXml(blocks: HtmlBlock[]): string {
  const fallbackBlock: HtmlBlock = {
    type: 'paragraph',
    segments: [{ text: '', style: {} }],
  };
  const paragraphXml = (blocks.length ? blocks : [fallbackBlock]).map((block) =>
    buildDocxParagraph(block),
  );
