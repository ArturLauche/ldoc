...
export function createLegalCopies(
  config: Pick<typeof siteConfig, 'siteName' | 'siteUrl' | 'jurisdiction'>,
) {
  const v = {
  ...
  return { privacyEn, privacyDe, termsEn, termsDe };
}

const copies = createLegalCopies(siteConfig);
export function getPrivacyCopy(locale: Locale): LegalCopy {
  return locale === 'de' ? copies.privacyDe : copies.privacyEn;
}

export function getTermsCopy(locale: Locale): LegalCopy {
  return locale === 'de' ? copies.termsDe : copies.termsEn;
}
